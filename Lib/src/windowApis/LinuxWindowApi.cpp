...
namespace ImGUIWindow {
	...
	bool LinuxWindowApi::Init(bool* runPtr, std::vector<fontWraper>& fonts) {
		...
		glfwWindow = glfwCreateWindow(1280, 720, windowTitle.c_str(), NULL, NULL);

		glfwMakeContextCurrent(glfwWindow);

		glfwSetWindowCloseCallback(glfwWindow, &LinuxWindowApi::glfw_closeWindow_callback);
		glfwSetWindowSizeCallback(glfwWindow, &LinuxWindowApi::glfw_handleResizeWindow_callback);
...
#	error "no 'ImGuiThemeX' defined, define one and replace 'X' with 'Light', 'Dark', or 'Classic'"
	...
	void LinuxWindowApi::BeginFrame() {
		glfwPollEvents();
	...
	void LinuxWindowApi::EndFrame() {
		ImGui::Render();
		ImDrawData* main_draw_data = ImGui::GetDrawData();
		const bool main_is_minimized = (main_draw_data->DisplaySize.x < 1 || main_draw_data->DisplaySize.y < 1);
	...
	void LinuxWindowApi::glfw_handleResizeWindow_callback(GLFWwindow* win, int w, int h) {
		int& width = Get()->w;
		int& height = Get()->h;
	...
	void LinuxWindowApi::SetupVulkan(const char** extensions, uint32_t extensions_count) {
		VkResult err;
	...
	void LinuxWindowApi::SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height) {
		wd->Surface = surface;
	...
	void LinuxWindowApi::CleanupVulkan() {
	...
	void LinuxWindowApi::CleanupVulkanWindow() {
		ImGui_ImplVulkanH_DestroyWindow(g_Instance, g_Device, &g_MainWindowData, g_Allocator);
	}

	void LinuxWindowApi::FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data) {
		VkResult err;
