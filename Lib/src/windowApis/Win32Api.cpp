...
namespace ImGUIWindow {
	...
	bool Win32Api::Init(bool* runPtr, std::vector<fontWraper>& fonts) {
		...
		glfwSetWindowCloseCallback(glfwWindow, &Win32Api::glfw_closeWindow_callback);
		glfwSetWindowSizeCallback(glfwWindow, &Win32Api::glfw_handleResizeWindow_callback);
...
#	error "no 'ImGuiThemeX' defined, define one and replace 'X' with 'Light', 'Dark', or 'Classic'"
	...
	void Win32Api::glfw_handleResizeWindow_callback(GLFWwindow* win, int w, int h) {
		int& width = Get()->w;
		int& height = Get()->h;
